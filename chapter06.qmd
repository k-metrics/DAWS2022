...
code <- c("JP90C000BE38", "JP90C0002LP2", "JP90C0002LR8", "JP90C0000R71",
          "JP90C000EMH6",
          "JP90C0002H06", "JP90C0009E44", "JP90C0007GF2", "JP90C0007GD7",
          "JP90C000A1B6",
          "JP90C000GKC6", "JP90C000FCD3", "JP3047260009", "JP90C000FFX4",
          "JP90C000FN66")
...
df %>% 
  ...
  ggplot2::ggplot(ggplot2::aes(x = `リスク（標準偏差）`, y = 騰落率,
                  color = code)) + 
    ggplot2::geom_abline(slope = 1, intercept = 0, size = 0.2) + 
    ggplot2::geom_point() + 
    # ggplot2::coord_fixed() +
...
nisa_id <- xml2::read_html(url) %>% 
  rvest::html_element(xpath = xpath) %>% 
  rvest::html_nodes("a") %>% 
  rvest::html_attr("href") %>% 
  as.data.frame() %>% 
  dplyr::rename(id = 1) %>% 
  dplyr::mutate(id = stringr::str_sub(id, start = 22))
...
nisa_id %>% 
  ...
  # readr::write_excel_csv("./data/NISA.csv")
```
...
nisa_info <- tidyr::crossing(url, code, xpath) %>% 
  dplyr::mutate(url = paste0(url, code)) %>% 
  purrr::pmap_df(.f = function (url, xpath, code) {
    xml2::read_html(url) %>% 
      rvest::html_element(xpath = xpath) %>% 
...
nisa_info # %>% 
  # readr::write_excel_csv("./data/NISA_手数料.csv")
```

```{r}
#| code-summary: "つみたてNISA運用情報"
xpath <- c('//*[@id="tab_content2"]/div/div[2]/div[1]/div[1]/div/table',
           '//*[@id="tab_content2"]/div/div[2]/div[1]/div[2]/div/table',
           '//*[@id="tab_content2"]/div/div[2]/div[1]/div[3]/div/table')
...
df %>% 
  ...
  dplyr::mutate(X3 = stringr::str_remove(X3, "%"),
                ...
                X2 = forcats::fct_inorder(X2)) %>% 
  tidyr::pivot_wider(names_from = X1, values_from = X3) # %>% 
  # readr::write_excel_csv("./data/NISA_RRS.csv")
```
...
#| eval: false
#| include: false
#| code-summary: "つみたてNISA運用情報（接続拒否か？）"
url <- "https://www.rakuten-sec.co.jp/web/fund/detail/?ID="
xpath <- '//*[@id="infoPage"]/div[6]/div/table'
code <- nisa_id$id

xml2::read_html(paste0(url, "JP90C0006LC1")) %>%
  rvest::html_element(xpath = xpath) %>%
  rvest::html_table(header = TRUE)
