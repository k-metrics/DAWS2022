...
　本講はテキスト前半部分の概観を説明する講義における演習をRで解答しています。テキストとは異なるデータを利用しており、演習自体やデータの著作権は講義担当者にあります。
...
　層別のヒストグラムを描くには**`ggplot2`**パッケージが便利です。ここでは階級幅を$10点$としています。
```{r}
#| include: false
#| eval: false
#| code-fold: true
...
"./data/Pre_P13_ツールを導入すべきか.csv" %>% 
  read.csv() %>% 
  with(t.test(変更前, 変更後, var.equal = TRUE))
```
